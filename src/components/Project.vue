<template>
    ...
    <div class=" flex flex-col justify-center items-center gap-2 w-full p-2 border rounded-lg shadow-md ">
        <div @click="showProject(project.id, project.status, 'done')"
            class=" project_content flex flex-col sm:flex-row justify-between items-start gap-4 w-full h-full ">
            ...
            <div class=" flex flex-col justify-between items-start w-full gap-2 ">
                <div class=" flex flex-row justify-between items-center w-full  ">
                    <div class=" flex flex-col justify-start items-start">
                        <h2 class=" text-base md:text-xl text-black/70 font-semibold ">
                            {{ useLanguageStore().getWord(useCategoriesStore().getCategoryById(project.categoryId)) }}</h2>
                        <h2 class=" text-sm md:text-base text-black/50 font-semibold mb-2 ">
                            {{ '#' + project.id.slice(0, 15) + '...' }}</h2>
                        <div class=" flex flex-row justify-start items-center space-x-2 w-full text-xs ">
                            <p class="text-gray-400 flex items-center "><span class="material-icons text-sm text-gray-400">
                                    location_on
                                </span>{{ useWilayasStore().getWilayaById(project.stateId) }}</p>
                            <p class="text-gray-400">{{ formatTime(project.createdAt) }}</p>
                            <!-- isPubllic -->
                        </div>
                    </div>
                ...
                <p class=" text-xs text-gray-400 font-semibold ">{{ project.description.length > 150 ?
                    project.description.slice(0, 150) + '...' : project.description }}</p>
        ...
        <div class=" project_action flex items-center justify-end w-full h-full py-1 gap-2 ">
            <button @click="showProject(project.id, project.status, 'pending')" v-if="project.status === 'pending'"
...
const showProject = (clientPostId, postStatus, status) => {

    if (postStatus === status) {
        clientDemandeStore.clientPostPageVisibility = true;
        clientDemandeStore.$state.selectedPost = props.project
