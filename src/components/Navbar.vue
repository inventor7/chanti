    ...
    <div :data-theme="theme"  dir="ltr" class="navbar border-b-2 -xl mb-8 fixed  top-0  fd  py-1 bg-base-100  z-50 " ref="navbar">
        <div class="navbar-start pl-2 ">
            ...
            <router-link v-if="!authStore.$state.isAuthenticated" :to="{ name: 'home' }">
                <svg :class="!themeStore.theme ? 'fill-primary' : 'fill-secondary'"
                    class="  px-0 w-20 h-auto object-contain btn btn-link font " data-v-423bf9ae=""
            ...
            <!-- when not loggedin -->
            <button v-else @click="handleGetNotification" class=" btn-ghost">
                <div class="indicator  ">
                    <span class="material-icons text-secondary text-4xl ">
                        circle_notifications
                    </span>
                    <span v-if="notificationStore.$state.notificationsNumber > 0"
                        class="badge p-2 m-1  badge-xs badge-primary indicator-item animate-pulse ">
        ...
        <div v-if="authStore.$state.isAuthenticated" class="navbar-center">
            <router-link :to="{ name: 'home' }">
                <svg class="  px-0 w-20 h-auto fill-secondary object-contain btn btn-link font " data-v-423bf9ae=""
                    xmlns="http://www.w3.org/2000/svg" viewBox="0 0 224.78573364505993 60">
                    <g data-v-423bf9ae="" id="4543de12-aaf0-4125-a132-5fbdb5bbcecc"
                        transform="matrix(5.357142948374458,0,0,5.357142948374458,-5.035717750082384,-9.000001226152698)">
                        <path
...
export default {
    ...
    setup() {
        ...
        const handleGoProfile = () => {
            portfolioStore.getProviderPortfolio(authStore.$state.userAuth).then((res) => {
                console.log(res)
            })
            router.push({
                name: 'profile',
