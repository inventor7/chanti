<template>
  <nav>
    <div dir="ltr" class="navbar shadow-lg  fixed fd  py-0 top-0 bg-base-100  z-10 " ref="navbar">
      ...
      <div class="navbar-end gap-2  md :gap-4">
        ...
        <button v-else @click="logout()" class=" btn btn-primary btn-xs gap-2 sm:btn-sm btn-outline ">
          ...
          <p class=""> {{ languageStore.getWord('logout') }} </p>
        </button>
...
export default {
  ...
  mounted() {
    window.addEventListener('scroll', () => {
      ...
      if (this.$refs.navbar && isScrollingDown && currentScrollPosition > 500) {
        const navbar = this.$refs.navbar;
        navbar.classList.add('hiddenn');
      } else if (this.$refs.navbar) {
        const navbar = this.$refs.navbar;
        if (navbar.classList.contains('hiddenn'))
